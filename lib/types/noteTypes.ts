 ...
 * Each note type has an icon (SVG markup) and a type identifier.
 */
export interface NoteType {
  /** SVG markup for the note icon */
  icon: string
  
  /** Type identifier (should match the key in NoteTypes) */
  type: string
}

/**
 * Collection of all available note types.
 * Keys are the note type identifiers (lowercase) that users can use in markdown.
 * 
 * @example
 * ```markdown
 * > [!note]
 * > This uses the 'note' type
 * 
 * > [!tip]
 * > This uses the 'tip' type
 * ```
 */
...
/**
 * Built-in note types supported by the plugin.
 */
export const NOTE_TYPES: NoteTypes = {
  'note': {
